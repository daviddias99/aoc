...
local function expandSpace(space)
  local lowerZPlane = 1
  local highZPlane = #space

  for y, row in ipairs(space[highZPlane]) do
    for x, value in ipairs(row) do
      if value == 1 then
        table.insert(space, #space + 1, create2DTableWithSize(#space[highZPlane], #row))
  ...
  for y, row in ipairs(space[lowerZPlane]) do
    for x, value in ipairs(row) do
      if value == 1 then
        table.insert(space, 1, create2DTableWithSize(#space[lowerZPlane], #row))
...
for y, line in ipairs(lines) do
  local x = 1
  table.insert(space[1], y, {})
  for element in string.gmatch(line,'.') do
    table.insert(space[1][y], x, element == '#' and 1 or 0)
    x = x + 1
  end
end
