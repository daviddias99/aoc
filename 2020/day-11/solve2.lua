...
local function calculateAdjacentToPos(grid, row, column)
  ...
  if row > 1 then
    ...
    if column > 1 then
      local r = row - 1
      local c = column - 1
      while r > 1 and c > 1 and grid[r][c] == '.' do
          r = r - 1
          c = c - 1
      end

      res = res + (grid[r][c] == '#' and 1 or 0)
    end

    local r = row - 1

    while r > 1 and grid[r][column] == '.' do
      r = r - 1
    end

    res = res + (grid[r][column] == '#' and 1 or 0)

    if column < #grid[1] then
      local r = row - 1
      local c = column + 1

      while r > 1 and c < #grid[1] and grid[r][c] == '.' do
        r = r - 1
        c = c + 1
      end
  ...
  if column > 1 then
    ...
    while c > 1 and grid[row][c] == '.' do
      c = c - 1
    end

    res = res + (grid[row][c] == '#'  and 1 or 0)
  end
  ...
  if column < #grid[1] then
    ...
    while c < #grid[1] and grid[row][c] == '.' do
      c = c + 1
    end
  ...
  if row < #grid then
    ...
    if column > 1 then
      ...
      local r = row + 1
      local c = column - 1
      while r < #grid and c >1 and grid[r][c] == '.' do
          r = r + 1
          c = c - 1
      end
      res = res + (grid[r][c] == '#'  and 1 or 0)
    ...
    local r = row + 1
    while r < #grid and grid[r][column] == '.' do
        r = r + 1
    end
    res = res + (grid[r][column] == '#'  and 1 or 0)
    ...
    if column < #grid[1] then
      local r = row + 1
      local c = column + 1
      while r < #grid and c < #grid[1] and grid[r][c] == '.' do
          r = r + 1
          c = c + 1
      end
      res = res + (grid[r][c] == '#'  and 1 or 0)
  ...
  for i = 1, #grid do
    ...
    for j = 1, #row do
      if grid[i][j] == 'L' and calculateAdjacentToPos(grid, i, j) == 0 then
        nextGrid[i][j] = '#'
        changed = true
      elseif grid[i][j] == '#' and calculateAdjacentToPos(grid, i, j) >= 5 then
        nextGrid[i][j] = 'L'
        changed = true
      else
